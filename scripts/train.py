...
def train_v2_freeze(config, working_dir: str):
    """
    Main program flow, since v0.9.6
    """
    import sevenn.train.atoms_dataset as atoms_dataset
    ...
    ###########freeze code##################
    # attention: this assumes you set train_shift_scale=True
    # if not, you should unfroze parameters of 'reduce_total_enegy'
    log.writeline("\nFreezing all layers except the final head...")

    for param in model.parameters():
        param.requires_grad = False

    try:
        for param in model.reduce_input_to_hidden.parameters():
            param.requires_grad = True
        log.writeline("Unfroze parameters in 'reduce_input_to_hidden'.")

        for param in model.reduce_hidden_to_energy.parameters():
            param.requires_grad = True
        log.writeline("Unfroze parameters in 'reduce_hidden_to_energy'.")
    except AttributeError as e:
        log.writeline(f"ERROR: Could not find layer to unfreeze. Check model structure. Details: {e}")
        raise AttributeError(f"ERROR: Could not find layer to unfreeze. Check model structure. Details: {e}")
    ...
    for name, param in model.named_parameters():
        ...
        if param.requires_grad:
            log.writeline(f"[Trainable]: {name} ({param.numel()} params)")
            trainable_params_count += param.numel()

    log.writeline(f"\nTotal parameters: {total_params_count}")
    log.writeline(f"Trainable parameters: {trainable_params_count}")
    log.writeline(f"Percentage of trainable parameters: {100 * trainable_params_count / total_params_count:.2f}%")
    #########################################
    trainer = Trainer.from_config(model, config)
    if state_dicts:
        trainer.load_state_dicts(*state_dicts, strict=False)
