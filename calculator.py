...
class SevenNetCalculator(Calculator):
    ...
    def calculate(self, atoms=None, properties=None, system_changes=all_changes):
        ...
        #output = self.model(data) # origin version
        ################revised###########################
        head_start_layer_name = 'reduce_input_to_hidden'
    
        # 调用增强版的 forward 方法
        output, embeddings_tensor = self.model(
            data,
            return_embeddings_at_layer=head_start_layer_name
        )
    
         # 步骤 3: 处理和存储所有结果
        atomic_embeddings = embeddings_tensor.detach().cpu().numpy()
        ##################################################
