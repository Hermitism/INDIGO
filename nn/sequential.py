...
class AtomGraphSequential(nn.Sequential):
    ...
    def forward(
        self,
        input: AtomGraphDataType,
        return_embeddings_at_layer: Optional[str] = None,
    ) -> Union[AtomGraphDataType, Tuple[AtomGraphDataType, torch.Tensor]]:
        data = input
        captured_embeddings: Optional[torch.Tensor] = None
        ...
        # 步骤 2: 遍历模块，同时检查是否需要捕获 embedding
        for name, module in self._modules.items():
            # 在运行这一层之前，检查是否是我们要捕获 embedding 的地方
            if name == return_embeddings_at_layer:
                captured_embeddings = data[self.key_node_feature]

            # 运行当前模块
            data = module(data)

        # 步骤 3: 根据是否需要返回 embedding，决定返回值
        if return_embeddings_at_layer is not None:
            if captured_embeddings is None:
                raise ValueError(
                    f"Layer '{return_embeddings_at_layer}' not found in model."
                )
            return data, captured_embeddings
        else:
            return data
